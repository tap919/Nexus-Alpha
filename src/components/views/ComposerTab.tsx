...
export const ComposerTab = () => {
  const [pattern, setPattern] = useState<string>('react-component');
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await fetch('/api/autocoder/generate', {
        ...
        body: JSON.stringify({ pattern, name: name.trim() }),
      });
      if (!response.ok) {
      ...
      if (data.success) {
        setChanges(data.files.map((f: any, i: number) => ({
          id: `change-${i}`,
          filePath: f.path,
          originalContent: '// Generated by Cheetah V3',
          newContent: f.content,
          status: 'pending'
        })));
        setLore(`Cheetah Engine: Synthesized ${data.pattern} using ${data.engine}. Saved ${data.tokenSavings} tokens.`);
        setThoughts(['Synthesis complete. Proposed changes ready.']);
      } else {
      ...
      <div className="relative glass-card rounded-2xl p-4 flex flex-col gap-4">
        <div className="flex gap-4">
          ...
          >
            ...
            <option value="crud-api">CRUD API Route</option>
            <option value="hono-route">Hono Route</option>
            <option value="typescript-interface">TS Interface</option>
            <option value="database-schema">DB Schema</option>
            <option value="supabase-table">Supabase Table</option>
            <option value="hook">React Hook</option>
          ...
          <input
            ...
            placeholder="Entity name (e.g. UserProfile, auth-hook)"
        ...
        <div className="flex items-center justify-between mt-2">
          ...
          >
            {isGenerating ? 'Synthesizing...' : 'Generate with Cheetah'}
